...
//   }Transactions`;
//   document.getElementById("transactionTypeLabel").textContent =
//     type.charAt(0).toUpperCase() + type.slice(1);
...
// });
 ...
 document.addEventListener("DOMContentLoaded", () => {
  const type = document.getElementById("type")?.value || "income";
  const endpoint = `http://localhost/Ei_backend/api/${type}.php`;
  ...
  async function fetchTransactions() {
    try {
      ...
      if (filters.category && filters.invoiceNo && filters.productName) {
...
// Search filter
  ...
  if (searchBox) {
    ...
    // Bind delete if form exists (means we're on Add Income page)
    if (form) {
  ...
  // Form submit handler (Add Income page only)
  if (form) {
    form.addEventListener("submit", async (e) => {
      e.preventDefault();
      ...
      select.addEventListener("change", () => {
        const filtered = allTransactions.filter((tx) =>
          (filters.category.value === "all" ||
            tx.category === filters.category.value) &&
          (filters.invoiceNo.value === "all" ||
            tx.invoiceNo === filters.invoiceNo.value) &&
          (filters.productName.value === "all" ||
            tx.productName === filters.productName.value)
        );
        renderTable(filtered);
      });
    }
